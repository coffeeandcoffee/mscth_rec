...
Statistical significance testing on the $n=25$ dataset using our primary clinical metric (Recall vs 50% chance, one-sample t-test) yields a p-value of $1.05 \times 10^{-12}$ and a massive effect size (Cohen's d = 2.70). **Therefore, $n=25$ is more than scientifically sufficient and constitutes the baseline thesis dataset.** Collecting data for up to $n=30$ is an optional stretch goal.
