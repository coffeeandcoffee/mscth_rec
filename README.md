...
1. **Dataset Freeze ($n=25$ Baseline, $n=30$ Optional)**: Statistical significance testing on the current $n=25$ dataset (one-sample t-test: Acc=65.7% vs 50% chance) yields $p = 9.74 \times 10^{-12}$ and Cohen's $d = 2.43$ (a massive effect size). **Scientifically, $n=25$ is more than sufficient to support the thesis.** Recruiting 5 more participants to hit $n=30$ is now an optional stretch target.
