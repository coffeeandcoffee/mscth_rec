...
| 12 | `20260311_221000_baseline_normalized_statistical_significance` | **Mathematical Proof**: Calculates one-sample t-test (vs 50% baseline) and Cohen's d for the newly generated baseline normalized RF model (Step 11). | ✅ |
| 13 | `20260311_222000_baseline_normalized_logo_cv_rf` | **Baseline Normalized LOGO-CV**: Re-runs the Step 10 LOGO-CV pipeline using Baseline Normalization to test if removing absolute amplitude bias allows the cross-participant learned state maps to generalize. | ✅ |
| 14 | `20260311_233500_engagement_index_prediction` | **EI Two-Class Prediction**: Trains the identical RF architecture natively on the 5 Standard Engagement Index values (`beta/(alpha+theta)`) to evaluate if traditional metrics can perform short-term micro-decision predictions. Serves as the negative control justifying the 112-feature structural pipeline. | ✅ |
| 15 | `20260311_235800_targeted_explainability` | **Targeted Explainability (Feature Importance)**: Extracts Mean Gini feature importances across the 25 Baseline Normalized RF models to identify exactly which neural regions and frequency bands drive predictive performance. | ✅ |
