...
---------------------------
We can state with extreme statistical significance ($p = 1.76 \\times 10^{-8}$, Cohen's $d = 1.65$) that the null hypothesis (random chance $50\\%$) is rejected in favor of the alternative hypothesis; the selected RF-112 machine learning pipeline successfully detects the neurological signature of 'about_to_skip' behavior prior to motor execution natively across an independent cohort of $N=25$ participants at a mean recall of $67.1\\%$.
