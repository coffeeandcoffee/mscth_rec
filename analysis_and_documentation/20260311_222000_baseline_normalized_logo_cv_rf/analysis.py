#!/usr/bin/env python3
"""
Baseline Normalized LOGO-CV — Step 13
=====================================
Evaluates the Baseline Normalized Random Forest (112 features) using LOGO-CV 
to test clinical generalizability across the 25 independent participants. 
For each of the 25 participants, it:
  1. Locates the global 100s baseline to normalize all 25 participants' features.
  2. Trains the RF on the normalized data of the *other* 24 participants.
  3. Evaluates the RF on the completely unseen 1 held-out normalized participant.
...
def extract_participant_data(pid, sub_recordings):
    ...
    if baseline_means is None:
        print(f"  ❌ CRITICAL: Could not locate a valid 100s keypress_B baseline for LOGO-CV participant {pid}. Skipping.")
        return None

    # 2. Extract and Normalize Samples
    for sr in sub_recordings:
        csv_name = sr["file"]
        blocks = sr["blocks"]
        csv_path = DATA_DIR / csv_name
...
def write_description(results: dict):
    ...
    lines = [
        "=" * 70,
        "BASELINE-NORMALIZED LEAVE-ONE-GROUP-OUT (LOGO) CROSS VALIDATION",
        "=" * 70, "",
        "OBJECTIVE", "-" * 9,
        "To test the clinical generalizability of the neurological signature",
        "under Baseline Normalization. Instead of intra-subject validation (training",
        "and testing on the same person), LOGO-CV enforces training on completely",
        "separate individuals and evaluating on a strictly unseen held-out participant.",
        "This checks if removing the absolute amplitude bias allows the cross-participant",
        "learned state maps to generalize.", "",
        "MODEL CONFIG", "-" * 12,
        f"  Model:            RandomForestClassifier",
        f"  Normalization:    Baseline Relative Power (100sec rest)",
    ...
    lines += [
        "", "=" * 70,
        "Generated by analysis.py (Step 13: Baseline Normalized LOGO-CV RF)",
        "=" * 70,
    ]
...
def main():
    print("=" * 60)
    print("STEP 13: BASELINE NORMALIZED LOGO-CV GENERALIZABILITY (n=25)")
    print("=" * 60)
