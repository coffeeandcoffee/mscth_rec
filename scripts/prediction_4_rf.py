#!/usr/bin/env python3
"""
Prediction V4: Skip Prediction with Random Forest

Predicts if user is about to skip (within next 3s) from EEG data using Random Forest.
...
Usage:
    python prediction_4_rf.py                        # Default 3s window
    python prediction_4_rf.py --window 3.0 --nonotch
"""
...
def train_random_forest(X_train, y_train, X_val, y_val, n_estimators=100, max_depth=10, min_samples_leaf=5):
...
def plot_confusion_matrix(labels, preds, output_dir):
    ...
    plt.colorbar(im)
    plt.tight_layout()
    plt.savefig(output_dir / 'confusion_matrix_rf.png', dpi=150)
    plt.close()
...
def plot_feature_importance(clf, feature_names, output_dir):
    ...
    ax.set_title('Top 20 Features: Random Forest Skip Prediction')
...
def plot_class_distribution(y_train, y_val, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'class_distribution_rf.png', dpi=150)
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Skip Prediction V4: Random Forest')
    ...
    print("=" * 60)
    print("PREDICTION V4: SKIP PREDICTION (RANDOM FOREST)")
    print("=" * 60)
    ...
    try:
        ...
        clf, results = train_random_forest(
        ...
        output_dir = input_file.parent / f"model_output_pred_v4_RF_{timestamp_str}"
        ...
        # Save trained model
        joblib.dump(clf, output_dir / 'random_forest_model.pkl')
        print(f"   ✓ Trained model saved (random_forest_model.pkl)")
