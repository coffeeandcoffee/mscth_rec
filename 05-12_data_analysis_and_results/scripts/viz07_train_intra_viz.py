...
def run(run_dir, params):
    ...
    for pid in config.INCLUDED_PARTICIPANTS:
        ...
        pdata.append({'pid': pid, 'tb': np.mean(rec_tb), 'rs': np.mean(rec_rs),
    ...
    bp1 = ax.boxplot([tb_vals], positions=[1], widths=0.35, patch_artist=True)
    bp2 = ax.boxplot([rs_vals], positions=[2], widths=0.35, patch_artist=True)
    bp1['boxes'][0].set_facecolor(PALETTE[0]); bp2['boxes'][0].set_facecolor(PALETTE[1])
    for d in pdata:
        ax.plot([1, 2], [d['tb'], d['rs']], color='gray', alpha=0.3, linewidth=0.5)
    ax.set_xticks([1, 2]); ax.set_xticklabels(['Temporal\nBlocked', 'Random\nSplit'])
    ...
    ax.set_xticks([1, 2, 3]); ax.set_xticklabels(['Full\n(112)', 'Frontal\n(56)', 'Temporal\n(56)'])
    ax.axhline(0.5, color='gray', linestyle='--', linewidth=0.5)
