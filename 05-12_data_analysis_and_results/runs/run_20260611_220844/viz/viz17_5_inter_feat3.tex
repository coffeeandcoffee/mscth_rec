\begin{table}[H]
\centering
\begin{tabular}{lccc}
\hline
Metric & EI LR & \multicolumn{2}{c}{TP10 raw std} \\\\
 & Val & Val & Sig \\\\
\hline
F1 Train (STAY) & 62.0\% & 49.6\% & TRUE \\\\
F1 Test (STAY) & 62.5\% & 46.0\% & TRUE \\\\
Precision (STAY) & 49.4\% & 47.5\% & FALSE \\\\
Recall (STAY) & 89.2\% & 51.0\% & TRUE \\\\
F1 Train (SKIP) & 20.6\% & \textcolor{green}{52.4\%} & TRUE \\\\
F1 Test (SKIP) & 10.7\% & \textcolor{green}{44.3\%} & TRUE \\\\
Precision (SKIP) & 23.5\% & \textcolor{green}{43.0\%} & TRUE \\\\
Recall (SKIP) & 9.6\% & \textcolor{green}{50.8\%} & TRUE \\\\
\hline
\end{tabular}
\caption{Step 5 (Inter-Subject): Proposed Feature (TP10 raw std) vs EI. Significance tested using Wilcoxon signed-rank test against EI (two-sided, $\alpha=0.05$, N=25).}
\label{tab:metrics_5_inter_3}
\end{table}