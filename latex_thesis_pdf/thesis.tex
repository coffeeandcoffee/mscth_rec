...
% -- methods read until here -- continute reading, continute inserting INSERT: ...
% next step: document full recording script, preprocessing, and analysi pipelines.

\section{Universal Recording Pipeline}

\section{Universal Pre-Processing Pipeline}

The following subsections describe step-by-step exactly what the Python code in the \texttt{04-26\_data\_analysis\_and\_results} directory executes to process the EEG signals into uniform shape suitable for further analysis.

\subsection{Signal Preprocessing \& Normalization (\texttt{???.py})}
